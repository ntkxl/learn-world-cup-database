...
echo "$($PSQL "SELECT DISTINCT name FROM (SELECT winner_id FROM games WHERE year = 2014 and round = 'Eighth-Final' UNION SELECT opponent_id FROM games WHERE year = 2014 and round = 'Eighth-Final') AS SUB LEFT JOIN teams ON SUB.winner_id = teams.team_id ORDER BY name")"
